...
const echo = new Tone.FeedbackDelay('2n', 0.4);
...
equalizer.forEach((equalizerBand, i) => {
  if (i < equalizer.length - 1) {
    // Connect to next equalizer band
    equalizerBand.connect(equalizer[i + 1]);
  } else {
    // This is the last band, connect it to the echo
    equalizerBand.connect(echo.input);
  }
});
echo.toMaster();
echo.connect(delay);
delay.connect(Tone.context.destination.input);
